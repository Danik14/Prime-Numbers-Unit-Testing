...
func Test_isPrime(t *testing.T) {
	...
	// running tests for every created struct
...
func Test_checkNumbers(t *testing.T) {
	...
	for _, e := range tests {
		// using string.NewReader() to create a new Reader
		// to scan it
...
func Test_intro(t *testing.T) {
	...
	// in order to replace standard out
	r, w, _ := os.Pipe()
...
func Test_readUserInput(t *testing.T) {
	...
	// Imitating user input by writing
	// '1' and 'q' command
	_, err := stdin.Write([]byte("1\nq\n"))
	if err != nil {
		t.Fatalf("Test_readUserInput FAILED, error writing to buffer: %v", err)
	}
